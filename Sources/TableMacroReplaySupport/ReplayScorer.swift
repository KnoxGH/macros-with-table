...
public struct ReplayResult: Codable, Equatable {
    ...
    public func humanReadable() -> String {
        var lines = [
            "TableMacroReplay (\(mode.rawValue))",
...
public enum ReplayError: Error, LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .unsupportedProfileVersion(let version):
            return "profile version \(version) is unsupported; expected \(TableMacroProfile.currentVersion)"
        case .invalidProfileTopology:
...
public enum ReplayInput {
    static func decodeProfile(_ data: Data) throws -> TableMacroProfile {
        try decoder().decode(TableMacroProfile.self, from: data)
    }
    ...
    public static func loadProfile(at url: URL) throws -> TableMacroProfile {
...
/// Rescoring against feature vectors retained in a saved evaluation report,
/// so a change to the classifier can be checked against real captured taps
/// without a live microphone.
public enum ReplayScorer {
    public static func score(
        profile: TableMacroProfile,
    ...
    private static func validate(profile: TableMacroProfile, evaluation: EvaluationReport) throws {
        guard profile.version == TableMacroProfile.currentVersion else {
