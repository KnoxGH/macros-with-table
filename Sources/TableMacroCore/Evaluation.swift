...
/// A completed 60-tap (or partial) accuracy session, saved as JSON and CSV.
...
public enum ClassifierEvaluator {
    /// Leave-one-out cross-validation over the calibration set: each sample is
    /// held out, a fresh classifier trains on the rest, and the held-out
    /// sample is predicted. This is the calibration-consistency check, not a
    /// substitute for a genuinely held-out accuracy session.
