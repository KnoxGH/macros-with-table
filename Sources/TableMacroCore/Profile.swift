...
/// One saved desk: its trained classifier, calibration provenance, and the
/// four assigned actions. Versioned so older topologies fail decoding
/// cleanly rather than being silently reinterpreted.
public struct TableMacroProfile: Codable, Equatable, Sendable, Identifiable {
...
/// Reads and writes feature-only profile JSON. A file whose version does not
/// match `TableMacroProfile.currentVersion` is skipped before its zones are even
/// decoded, so a six- or nine-zone export can never be reinterpreted as the
/// current four-zone layout. A file that claims the current version but is
/// internally inconsistent is surfaced as an error instead of ignored.
public final class ProfileStore {
    ...
    public init(directory: URL? = nil, fileManager: FileManager = .default) throws {
        ...
        if let directory {
            ...
            self.directory = support.appendingPathComponent("TableMacro/Profiles", isDirectory: true)
        }
        try fileManager.createDirectory(at: self.directory, withIntermediateDirectories: true)
    }

    public func loadAll() throws -> [TableMacroProfile] {
        ...
        return try files.compactMap { url in
            ...
            guard envelope.version == TableMacroProfile.currentVersion else { return nil }
            let profile = try decoder().decode(TableMacroProfile.self, from: data)
    ...
    public func save(_ profile: TableMacroProfile) throws {
        var updated = profile
    ...
    public func delete(_ profile: TableMacroProfile) throws {
...
public final class EvaluationStore {
    ...
    public init(directory: URL? = nil, fileManager: FileManager = .default) throws {
        ...
        if let directory {
            ...
            self.directory = support.appendingPathComponent("TableMacro/Evaluations", isDirectory: true)
        }
        try fileManager.createDirectory(at: self.directory, withIntermediateDirectories: true)
    }
...
public final class ApproachComparisonStore {
    ...
    public init(fileURL: URL? = nil, fileManager: FileManager = .default) throws {
        if let fileURL {
            ...
            let directory = support.appendingPathComponent("TableMacro", isDirectory: true)
