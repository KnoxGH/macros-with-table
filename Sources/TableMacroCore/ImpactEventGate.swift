...
/// Rejects obviously sustained acoustic events after the streaming onset
/// detector has captured a complete candidate. This is deliberately a
/// conservative gate: ambiguous short sounds continue to the profile
/// classifier, while speech-like events whose energy stays high across most
/// of the window are discarded before that classifier ever sees them.
enum ImpactEventGate {
    ...
    static func accepts(_ event: DetectedTap, sampleRate: Double) -> Bool {
        ...
        // A contact impact should emerge clearly from the immediately
        // preceding audio. This removes most consonant peaks that occur in
        // the middle of continuous speech without a general-purpose speech model.
        ...
        // together keeps the gate generous toward desks with a long resonant tail.
    ...
    static func metrics(for event: DetectedTap, sampleRate: Double) -> Metrics? {
        ...
        // elevated broadband room noise. This conservative peak alternative
        // preserves those candidates even when their 5 ms RMS is diluted.
