...
/// A low-allocation streaming onset detector. It adapts to the local noise
/// floor, retains a short pre-roll, and emits a fixed-length analysis window
/// whenever a short, high-contrast impulse rises above that floor.
public final class StreamingTapDetector {
    ...
    public func process(channels incoming: [[Float]]) -> [DetectedTap] {
        ...
        // The initial fixed floor is only a safe bootstrap value. A MacBook
        // mic in a real room can sit well above it; detecting before that
        // floor is learned would make every buffer look like an impulse and
        // the floor would never get a chance to rise.
        ...
        // Detect on a low-pass signal so the optional 15.5-21 kHz probe cannot
        // arm its own capture. The emitted event still contains the untouched
        // full-band channels used by the feature extractor.
    ...
    private func adaptNoiseFloor(to measuredRMS: Double, isWarmUp: Bool) {
        ...
        // cascade, but cap one-step growth so a real tap does not become the
        // new baseline. Downward movement is deliberately slower and steadier.
    ...
    private func completeCaptureIfReady() -> DetectedTap? {
        ...
        // Letting the floor follow it during the refractory period keeps
        // conversation from repeatedly re-arming the detector every 140 ms.
