...
/// A feature vector paired with the coarse frequency response of the same
/// window, so a caller who needs both never repeats the FFT.
...
/// Converts a detected analysis window into the named feature vector the
/// classifier trains and predicts on. Combines temporal shape, frequency
/// bands, mel-cepstral coefficients, and any spatial cues the selected
/// sensing strategy exposes.
public struct TapFeatureExtractor: Sendable {
    ...
    private func makeFeature(
        ...
        switch strategy {
        case .passive:
            selected = passiveFeatures(signal: mono, channels: channels, onset: event.onsetOffset, spectrum: powerSpectrum)
        case .active:
            selected = ActiveProbe.responseFeatures(signal: mono, sampleRate: sampleRate, spectrum: powerSpectrum)
        case .hybrid:
            let passive = passiveFeatures(signal: mono, channels: channels, onset: event.onsetOffset, spectrum: powerSpectrum)
            let active = ActiveProbe.responseFeatures(signal: mono, sampleRate: sampleRate, spectrum: powerSpectrum)
    ...
    private func prepare(_ event: DetectedTap) -> (channels: [[Float]], mono: [Double], powerSpectrum: [Double]) {
