...
/// A tiny deterministic xorshift-style PRNG so soak runs are reproducible.
...
let summary = String(
    format: "TableMacro soak complete: %.1fs, \(iteration) events; positives \(positiveCorrect) correct, \(positiveRejected) rejected, \(positiveWrong) wrong; rejection challenges \(rejectionChallengesPassed) rejected, \(rejectionChallengesFalseAccepted) false accepted; RSS %.1f→%.1f MB (Δ %.1f MB)",
