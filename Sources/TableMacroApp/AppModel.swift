...
enum TableMacroStorageError: Error, LocalizedError {
    case unavailable(String)

    var errorDescription: String? {
        switch self {
        case .unavailable(let area):
            return "\(area) storage is unavailable. TableMacro did not save this change."
        }
    }
}
...
final class AppModel: ObservableObject {
    ...
    @Published private(set) var profiles: [TableMacroProfile] = []
    ...
    var selectedProfile: TableMacroProfile? {
    ...
    func beginCalibration(draft: CalibrationDraft, recalibrating: TableMacroProfile? = nil) {
        pausedByUser = false
    ...
    func setDebugRecordingEnabled(_ enabled: Bool) {
        ...
        guard debugStore != nil else {
            debugRecordingEnabled = false
            errorMessage = TableMacroStorageError.unavailable("Debug recording").localizedDescription
            return
        }
    ...
    func clearDebugRecordings() {
        do {
            guard let debugStore else { throw TableMacroStorageError.unavailable("Debug recording") }
    ...
    private func handleBenchmark(_ observation: TapObservation, session: inout BenchmarkSession) {
        ...
        guard let nextStrategy = session.currentStrategy, let nextZone = session.currentZone else {
            do {
                ...
                guard let comparisonStore else { throw TableMacroStorageError.unavailable("Sensing comparison") }
    ...
    private func finishEvaluation(_ session: EvaluationSession) {
        ...
        evaluationSession = nil
        do {
            guard let evaluationStore else { throw TableMacroStorageError.unavailable("Evaluation report") }
    ...
    private func draft(for profile: TableMacroProfile) -> CalibrationDraft {
        CalibrationDraft(
            name: profile.name,
