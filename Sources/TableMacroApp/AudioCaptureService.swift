...
enum AudioCaptureError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .microphonePermissionDenied:
            return "Microphone access is required. Enable TableMacro in System Settings › Privacy & Security › Microphone."
        case .noInputChannels:
        ...
        case .builtInInputRequired(let selected):
            return "TableMacro requires the MacBook's built-in microphone. The current input is \(selected). Select MacBook Microphone in System Settings › Sound › Input, then press Resume."
...
final class AudioCaptureService: ObservableObject {
    ...
    nonisolated private let processingQueue = DispatchQueue(label: "com.tablemacro.audio-analysis", qos: .userInteractive)
