...
TableMacro is an experimental native macOS utility that turns the desk immediately around a MacBook into four assignable tap zones. It listens through the Mac's microphone, classifies each tap locally, and runs the action assigned to that zone.
...
TableMacro is a research prototype. Automated DSP tests pass, but useful accuracy still has to be measured on each real MacBook, desk, room, and laptop position. No physical accuracy claim is made without a saved 60-tap evaluation from that setup.
...
Then open `TableMacro.xcodeproj`, select the `TableMacro` scheme, and run it on My Mac. A new install requests microphone permission when calibration begins or when the user explicitly presses Resume; it does not prompt merely because the window opened.
...
4. Begin calibration. TableMacro prepares and arms the highlighted zone automatically.
...
7. The zone disarms after ten accepted samples. Move to the next highlighted zone during the short transition; TableMacro arms it automatically. Sounds made during the transition are ignored. If listening is paused, use the visible Arm control after resuming.
...
Audio processing happens on the Mac. TableMacro does not upload audio, features, profiles, or reports. An assigned website or application action can of course open that external destination.
