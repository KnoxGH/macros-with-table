...
archive_path="$build_dir/TableMacro.xcarchive"
app_path="$archive_path/Products/Applications/TableMacro.app"
dmg_root="$build_dir/dmg-root"
dmg_path="${DMG_PATH:-$repo_root/dist/TableMacro.dmg}"
...
log "Generating TableMacro.xcodeproj"
xcodegen generate
...
[[ -d "$app_path" ]] || fail "archive did not contain TableMacro.app at $app_path"
...
log "Signing TableMacro.app with sandbox entitlements and hardened runtime"
codesign \
    --force \
    --options runtime \
    --timestamp \
    --entitlements "$repo_root/Config/TableMacro.entitlements" \
    --sign "$DEVELOPER_ID_APPLICATION" \
    "$app_path"
...
mkdir -p "$dmg_root"
ditto "$app_path" "$dmg_root/TableMacro.app"
